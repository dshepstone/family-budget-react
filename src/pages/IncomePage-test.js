...
const IncomePage = () => {
  ...
  const ACCOUNT_OPTIONS = Array.from(
    new Set([...accountsFromState, ...accountsFromMonthly, ...accountsFromAnnual, ...DEFAULT_ACCOUNTS])
  );
  ...
  }, [ACCOUNT_OPTIONS]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const handleEdit = (index) => {
    ...
    setFormData({
      ...
      frequency: income.frequency || 'monthly',
      account: income.account || ACCOUNT_OPTIONS[0] || DEFAULT_ACCOUNTS[0],
      notes: income.notes || '',
  ...
  const resetForm = () => {
    setFormData({
      ...
      projectedAmount: '',
      actualAmount: '',
      account: ACCOUNT_OPTIONS[0] || DEFAULT_ACCOUNTS[0],
      notes: '',
      weeks: Array(5).fill(0),
      actualWeeks: Array(5).fill(0),
      payDates: [],
