...
const IncomePage = () => {
  ...
  const QuickEntrySection = () => {
    const incomeSources = incomeData.filter(income => getMaxPayDates(income.frequency) > 0);
      ...
      return (
        <div className="quick-entry-section">
          <div className="quick-entry-no-sources">
            ...
            <p>Add income sources with weekly, bi-weekly, or monthly frequencies to use quick paycheque entry.</p>
          </div>
        </div>
      );
    }
      ...
      <div className="quick-entry-section">
        ...
        <div className="quick-entry-grid">
          {incomeSources.map((income) => {
            const maxDates = getMaxPayDates(income.frequency);
            // Use stable key based on id or name
              ...
              <div key={incomeKey} className="quick-entry-card">
                    ...
                    return (
                      <div key={`${incomeKey}:row:${index}`} className="quick-entry-pay-row">
                        ...
                        <input
                          ...
                          pattern="[0-9]*[.,]?[0-9]*"
                          autoComplete="off"
                          className={`quick-entry-amount-input ${income.payActuals?.[index] !== undefined ? 'has-existing-data' : ''}`}
                          placeholder="Amount received"
                          value={currentAmount}
                          onChange={(e) => handleQuickEntryAmountChange(incomeKey, index, e.target.value)}
                          onBlur={() => handleQuickEntryAmountBlur(incomeKey, index)}
                        />
                        <button
                ...
                <div className="quick-entry-summary">
                  <div className="quick-entry-summary-item">
                    ...
                    <span className="quick-entry-summary-value">{formatCurrencyUtil(projectedMonthly)}</span>
                  </div>
                  <div className="quick-entry-summary-item">
                    <span className="quick-entry-summary-label">Actual:</span>
                    <span className="quick-entry-summary-value">{formatCurrencyUtil(actualMonthly)}</span>
                  </div>
                </div>
              </div>
            );
